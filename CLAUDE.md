...
## Domain architecture: hovanchuong.site (standalone domain + basePath, no gateway)

Unlike the base `sunshine-sky-city` boilerplate this lineage started from, this project does **not** run behind the `canhoquan7-gateway` rewrite project. It owns its own domain, `hovanchuong.site`, assigned directly to this app's own Vercel project (`hovanchuongklb-9827s-projects/noble-crystal-riverside`). There is no separate gateway repo involved and nothing to add there.
...
- `siteConfig.url` is the full external URL (`https://hovanchuong.site/noble-crystal-riverside`, no trailing slash) used for canonical/OpenGraph. `siteConfig.seo.ogImage` is a **full absolute URL**, not a root-relative path — combining a leading `/` with `metadataBase` resolves against the domain root and silently drops the `/noble-crystal-riverside` segment. `metadataBase` itself is constructed as `` new URL(`${siteConfig.url}/`) `` (trailing slash added) so any *future* relative metadata field resolves correctly instead of dropping the last path segment.

**Deployed**: the Vercel project points directly at the `hovanchuong.site` domain (Project → Settings → Domains) — no rewrites, no second repo. A request to the bare domain root (`hovanchuong.site/`, no `/noble-crystal-riverside`) 404s unless a redirect is added separately; that is expected with this setup, not a bug. Before the custom domain's DNS propagated, the project was reachable at its default `noble-crystal-riverside.vercel.app` (root basePath, no prefix) — that placeholder config is what the very first commits on this repo used.

**GitHub / Vercel / Supabase**: this project has its own GitHub repo (`hovanchuongklb-ladipage/hovanchuong`), its own Vercel project, and its own Supabase project — separate from both the base boilerplate's and the `chauthanhvuong` clone's. Never reuse credentials across these three projects.
